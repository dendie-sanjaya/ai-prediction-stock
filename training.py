...
import joblib # Import library joblib untuk menyimpan scaler
...
def evaluate_model_on_test(model, X_test, y_test, scaler, features_cols, target_col):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('prediksi_aktual_data_uji.png')
    plt.clf()
...
if __name__ == "__main__":
    ...
    model.save('model_saham_lstm.h5')
    print("Model berhasil disimpan sebagai 'model_saham_lstm.h5'")
    
    joblib.dump(scaler, 'scaler_saham.pkl')
    print("Scaler berhasil disimpan sebagai 'scaler_saham.pkl'")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('prediksi_harga_masa_depan.png')
    plt.clf()
